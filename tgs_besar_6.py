...
class ToDoApp:
    ...
    def create_widgets(self):
        ...
        self.priority_entry = tk.Entry(form_frame, font=('Arial', 12))
        self.priority_entry.grid(row=3, column=1, padx=5, pady=5)

        tk.Label(form_frame, text="Deadline (YYYY-MM-DD HH:MM):", bg='#282C34', fg='white', font=('Arial', 12)).grid(row=4, column=0, padx=5, pady=5)
        self.deadline_entry = tk.Entry(form_frame, font=('Arial', 12))
        ...
        view_button = tk.Button(button_frame, text="Lihat Tugas", command=self.view_tasks, bg='#98C379', fg='black', font=('Arial', 12))
    ...
    def edit_task(self):
        ...
        if selected_item:
            ...
            for task in tasks:
                if (task.title, task.description, task.category, task.priority, str(task.deadline), task.status) == task_values:
                    new_title = simpledialog.askstring("Edit Judul", "Judul baru:", initialvalue=task.title)
                    new_description = simpledialog.askstring("Edit Deskripsi", "Deskripsi baru:", initialvalue=task.description)
                    new_category = simpledialog.askstring("Edit Kategori", "Kategori baru:", initialvalue=task.category)
                    new_priority = simpledialog.askstring("Edit Prioritas", "Prioritas baru:", initialvalue=task.priority)
                    new_deadline_str = simpledialog.askstring("Edit Deadline", "Deadline baru (YYYY-MM-DD HH:MM):", initialvalue=task.deadline.strftime("%Y-%m-%d %H:%M"))
